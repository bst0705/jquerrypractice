...
ナビゲーションをつくる(ページ内リンク)
  ...
  offsetメソッド
    offsetメソッドを使用することで、要素の表示位置を取得することができる。
    offsetメソッドは、ページの上端からの距離とページの左端からの距離が連想配列の形で返ってきます。( {top: 〇〇, left: 〇〇} )
    offset().topとすると、ページの上端からの距離が取得できます。
    
  attrメソッドで飛び先を取得
    各ボタンはaタグにし、そのhref属性に飛び先のid名を指定しておく。
    そしてクリックしたときにattrメソッドでhref属性の値を取得し、
    offsetメソッドでその要素の位置を取得することでページ内リンクを実装できます。*/
